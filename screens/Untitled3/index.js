...
import { View, Text, ImageBackground, SafeAreaView, Pressable, FlatList } from "react-native";
...
const ProductDetail = ({
  ...
  const regex = /(<([^>]+)>)/gi;
  const {
    entities: Democonnector_response_get_productfeeds
  } = useSelector(state => state.Democonnector_response_get_productfeeds);
  ...
  const product = Democonnector_response_get_productfeeds ? response.filter(i => i.id === item) : exampleData.filter(i => i.id === item);
  const typeFilter = response?.filter(obj => {
    return obj?.data.attributes.type == product[0]?.data.attributes.type && obj?.data.attributes.gender == product[0]?.data.attributes.gender && obj?.id != product[0]?.id;
  });
  const [mainImage, setMainImage] = useState();

  const priceFormat = price => new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: "USD"
  }).format(price);
  ...
  return <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Pressable onPress={() => navigation.navigate("Untitled2", setFiltered(false))}>
          <ImageBackground source={{
          uri: "https://upload.wikimedia.org/wikipedia/commons/thumb/7/75/Levi%27s_logo.svg/77px-Levi%27s_logo.svg.png"
        }} style={styles.headerImage} />
        </Pressable>
        <ImageBackground source={require("../images/shopping_bag_FILL0_wght400_GRAD0_opsz24.png")} style={styles.bag} />
      </View>
      ...
      <View style={[styles.flexRow]}>
        <View style={styles.flexColumn1}>
          ...
          <FlatList data={product[0]?.data?.media} keyExtractor={(index, item) => index} numColumns={3} renderItem={({
          item
        }) => <Pressable onPress={() => setMainImage(item.src)}>
                <ImageBackground style={styles.pdpImagePr} source={{
            uri: item?.src
          }} />
              </Pressable>}></FlatList>
          {typeFilter != [{}] ? <>
          ...
          <FlatList data={typeFilter} keyExtractor={item.id} numColumns={5} renderItem={({
          item
        }) => <View>
        ...
        <View style={styles.flexColumn1}>
          ...
          <Pressable onPress={() => console.log(dispatch(api_v1_shopping_cart_item_create({
          item_sku: product[0].sku
        })))}>
...
const exampleData = [{
  ...
  data: {
    ...
    attributes: {
      type: "Jeans",
      gender: "Women",
      material: "92% Cotton, 6% Polyester, 2% Elastane (Stretch Denim), Zip fly with metal shank button closure, 5-pocket styling, Imported",
      brand: "Levi's",
      age_group: "Adult"
    },
    active: true,

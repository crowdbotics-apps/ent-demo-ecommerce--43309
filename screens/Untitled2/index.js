...
import { View, Text, FlatList, ImageBackground, Pressable, SafeAreaView } from "react-native";
...
const HomeScreen = ({
  ...
  const response = Democonnector_response_get_productfeeds ? Democonnector_response_get_productfeeds : exampleData;
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(democonnector_get_productfeedjson_list());
  }, []);

  const catFilter = catName => response?.filter(x => x.data?.categories.some(y => y.name == catName));
  ...
  return <SafeAreaView style={styles.container}>
      ...
      <View style={styles.subHeader}>
        <Pressable onPress={() => {
        setFiltered(true), setProdFilt(catFilter("men")), setFilterText("MEN");
      }}>
          <Text style={styles.links}>MEN</Text>
        </Pressable>
        <Pressable onPress={() => {
        setFiltered(true), setProdFilt(catFilter("women")), setFilterText("WOMEN");
      }}>
        ...
        {filtered === true ? <Pressable style={styles.reset} onPress={() => setFiltered(false)}>
            <ImageBackground style={styles.removeFilt} source={require("../images/close_FILL0_wght400_GRAD0_opsz24.png")}></ImageBackground>
            <Text style={styles.resetText}>{filterText}</Text>
          </Pressable> : null}
      </View>
      <FlatList data={filtered ? prodFilt : response} keyExtractor={item => item?.id} numColumns={5} renderItem={({
      item
